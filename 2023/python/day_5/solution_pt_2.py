...
if __name__ == "__main__":
    ...
    print(f"data load: {datetime.now() - now}")
    ...
    locations = []
    for curr_range in seed_ranges:
        print(curr_range)
        curr_seeds = range(curr_range[0], curr_range[0] + curr_range[1])
        with Pool(25) as p:
            locations += p.map(solve_for_seed, curr_seeds)
        locations = [min(locations)]
        print(locations)
    print(f"solve: {datetime.now() - now}")

    # Repeat, after humidity-to-location, store the answer!
    # Get min of this list
    # print(locations)
    print(f"Part Two Answer is: {min(locations)}")
